...
public class Spawner : MonoBehaviour
{
    ...
    private void SpawnSpawnable()
    {
        ...
        if (_classification && plane)
        {
            ...
            int noOfCubesX = ((int) Mathf.Ceil( dimensions.x / cubeSize ));
            int noOfCubesY = 20;

            float cubeSizeX = dimensions.x / noOfCubesX;
            float cubeSizeY = noOfCubesY * 0.1f < dimensions.y ? 0.1f : dimensions.y/noOfCubesY;
            ...
            for (int i = 0; i < noOfCubesY; i++)
            {
                for (int j = 0; j < noOfCubesX; j++)
                {
                    ...
                    cube.transform.localPosition = new Vector3((-(dimensions.x / 2)) + (j * cubeSizeX) + (cubeSizeX / 2), (-(dimensions.y / 2)) + (i * cubeSizeY) + (cubeSizeY / 2), -(cubeSize / 2));
            ...
            FindObjectOfType<GamePlay>().AddCubes(cubesToAdd, noOfCubesY, plane);
        }
